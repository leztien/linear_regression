...
"""
stochastic gradient descent for softmax classification 
"""
...
X,y = make_data_for_classification(m=500, n=30, k=3, 
...
class StochasticGradientDescent:
    ...
    def fit(self, X,y):
        ...
        #add bias feature and shuffle
        nx = np.random.RandomState(self.random_state).permutation(len(X))
        X = np.c_[np.ones(len(X))[:,None], X[nx]]
        y = y[nx]
        
        #get Ytrue into the correct shape (m, k)
        Ytrue = np.zeros(shape=(X.shape[0], k))
        ...
        for epoch in range(self.max_iter):
            ix = epoch % m
            x = X[ix][None,:]          # shape = (1, n+1)
            ytrue = Ytrue[ix][None,:]  # shape = (1, k)
            z = np.matmul(x, Θ)        # shape = (1, k)
            e = np.exp(z)              # shape = (1, k)
            ypred = e / e.sum()        # shape = (1, k)
            
            #penalty
            Θtemp = Θ.copy()
            Θtemp[0,:] = 0
            P = self.λ * Θtemp         # shape = (n+1, k)
            
            #gradient matrix
            G = x.T @ (ypred - ytrue) + P  # shape = (n+1, k)
            Θ = Θ - self.η * G
            
            #check cost
            if not(epoch % 5):
                J = self.cross_entropy(X,y, Θ, add_bias_feature=False)
                if J < 0.1:
                    print("breaking after epoch", epoch)
                    self.best_parameters = Θ
                    break  
        else: 
...
md = StochasticGradientDescent(random_state=None, verbose=True)
md.fit(X,y)
